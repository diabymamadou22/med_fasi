...
async function startServer() {
  ...
  // Media storage folder setup (persists shared videos for both partners)
  const MEDIA_DIR = path.join(process.cwd(), "uploads", "media");
  if (!fs.existsSync(MEDIA_DIR)) {
    fs.mkdirSync(MEDIA_DIR, { recursive: true });
  }

  // GET /api/media/:filename -> Streams media with HTTP Range (206 Partial Content) support
  ...
  // HEAD /api/media/:filename -> Returns media metadata (Content-Length, Accept-Ranges)
  app.head("/api/media/:filename", (req, res) => {
    const safeFilename = path.basename(req.params.filename);
    const filePath = path.join(MEDIA_DIR, safeFilename);
  ...
  // POST /api/media/upload -> Direct binary stream upload (works for files of any size)
  app.post("/api/media/upload", (req, res) => {
    ...
    const filename = `vid_${Date.now()}_${Math.random().toString(36).substring(2, 9)}${ext}`;
    ...
    writeStream.on("finish", () => {
      ...
      console.log(`[Media Upload] Sauvegarde réussie: ${filename} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);
      return res.json({
        success: true,
        url: fileUrl,
        filename,
    ...
    writeStream.on("error", (err) => {
      console.error("[Media Upload] Erreur d'écriture:", err);
      return res.status(500).json({ error: "Échec de l'enregistrement du média sur le serveur" });
    });

    req.on("error", (err) => {
      console.error("[Media Upload] Erreur de lecture requête:", err);
      return res.status(500).json({ error: "Échec de transmission du flux média" });
    });
  });

  // POST /api/media/upload-base64 -> Base64 dataUrl upload fallback
  app.post("/api/media/upload-base64", express.json({ limit: "100mb" }), (req, res) => {
    try {
      const { dataUrl, filename: reqFilename } = req.body;
      ...
      if (!matches || matches.length !== 3) {
        return res.status(400).json({ error: "Format DataURL invalide" });
      }
      ...
      else if (reqFilename) ext = path.extname(reqFilename) || ext;
      ...
      console.log(`[Media Upload Base64] Sauvegarde réussie: ${filename} (${(buffer.length / 1024 / 1024).toFixed(2)} MB)`);
      return res.json({
        success: true,
        url: fileUrl,
        filename,
      ...
      console.error("[Media Upload Base64] Erreur:", err);
      return res.status(500).json({ error: err.message || "Erreur de traitement du média base64" });
    }
  });
